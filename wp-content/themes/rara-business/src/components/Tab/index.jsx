		...
		return (
			<>
				<div className="get-pro">
					<img src={logo} alt={__('Logo', 'rara-business')} />
					<h3>{__('Rara Business Pro', 'rara-business')}</h3>
					<p>{__('Get access to all unlimited features.', 'rara-business')}</p>
					<a className="cw-button-btn primary-btn" target="_blank" href={cw_dashboard.get_pro}>{__('Get Pro', 'rara-business')}</a>
				</div>
			</>
		);
	}
